...
gen_header()
{
	...
	if [ "$enum" = 1 -a -z "$output_m4" ]; then
		echo "$1: error: #enum has been specified, but no m4 output " \
		     "file name has been provided" >&2
		exit 1
	fi
	...
	[ "$enum" != 1 ] || (
		...
		printf "dnl Generated by %s from %s; do not edit.\n" \
			"$0" "$input"
		printf "AC_DEFUN([%s],[\n" "st_CHECK_ENUMS_${name}"
		[ -z "$header" ] || printf "AC_CHECK_HEADERS([%s],[\n" "$header"
	...
	*) usage ;;
	...
	if [ -d "${input}" ]; then
		...
		for f; do
			...
			$pids
			EOF
		done | gen_m4_entry "${output}/st_check_enums.m4"
